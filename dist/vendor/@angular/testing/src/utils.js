...
var BrowserDetection = (function () {
    function BrowserDetection(ua) {
        if (lang_1.isPresent(ua)) {
            ...
            this._ua = lang_1.isPresent(platform_browser_private_1.getDOM()) ? platform_browser_private_1.getDOM().getUserAgent() : '';
        }
    }
...
function dispatchEvent(element, eventType) {
    platform_browser_private_1.getDOM().dispatchEvent(element, platform_browser_private_1.getDOM().createEvent(eventType));
}
exports.dispatchEvent = dispatchEvent;
function el(html) {
    return platform_browser_private_1.getDOM().firstChild(platform_browser_private_1.getDOM().content(platform_browser_private_1.getDOM().createTemplate(html)));
}
exports.el = el;
...
function stringifyElement(el) {
    var result = '';
    if (platform_browser_private_1.getDOM().isElementNode(el)) {
        var tagName = platform_browser_private_1.getDOM().tagName(el).toLowerCase();
        // Opening tag
        result += "<" + tagName;
        // Attributes in an ordered way
        var attributeMap = platform_browser_private_1.getDOM().attributeMap(el);
        var keys = [];
        ...
        result += '>';
        // Children
        var childrenRoot = platform_browser_private_1.getDOM().templateAwareRoot(el);
        var children = lang_1.isPresent(childrenRoot) ? platform_browser_private_1.getDOM().childNodes(childrenRoot) : [];
    ...
    else if (platform_browser_private_1.getDOM().isCommentNode(el)) {
        result += "<!--" + platform_browser_private_1.getDOM().nodeValue(el) + "-->";
    }
    else {
        result += platform_browser_private_1.getDOM().getText(el);
    }
    return result;
}
